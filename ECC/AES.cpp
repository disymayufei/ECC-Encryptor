...
/*Generate a KEY of 256 bits*/
std::string Gen_AES_Key() {
	unsigned char KeyArray[33];
	WinRandom r;
	Base64 bs;

	for (int i = 0; i < 32; ++i) {
		KeyArray[i] = r.randUnsignedChar();
	}

	return bs.Encode(KeyArray, 32);
}
...
std::string AES_Encrypt(std::string Mes, std::string Key) {
	...
	std::string temp_key = bs64.Decode(Key);

	for (int i = 0; i < 32; ++i) {
		Encrypt_Key[i] = temp_key[i];
	}
	...
	aes_key_expansion(Encrypt_Key, w);

	long long MSize = Mes.size();
	...
	return bs64.Encode((unsigned char*)Result.c_str(), Result.size());
}
...
std::string AES_Decrypt(std::string E_Mes, std::string Key) {
	...
	while (Result[Result.size() - 1] == '\a') {
		Result = Result.replace(Result.end()-1, Result.end(), "");
	}
